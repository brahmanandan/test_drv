...
def ProcessStateCases(options, Cases, GlobalStates, InitialLines, FunctionName=None, StateMachineName=None):
	# Cases includes keys:
	#     State (string)
	...
	DotHeader = 'digraph '
	if StateMachineName:
		DotHeader += StateMachineName
	DotHeader += '{'
	DotGraphLines.append(DotHeader)

	CorradinDotSetup = 'node [shape="Mrecord", fontname = "Courier New", fontsize=10, style = "filled, bold" fillcolor = "white"];' \
					+  ' edge [fontname = "Courier New", fontsize=8];'
	if options.corradin_format:
		DotGraphLines.append(CorradinDotSetup)
	...
	for case in Cases:
		...
		for commentline in case['CommentLines']:
			if InLabelBlock:
				...
				if m is not None:
					...
					Label = '<' + Label + '>'
					# Tables in Dot Graphs shouldn't have IDs
					if Label.startswith('<<table'):
						Label = TableIDRE.sub("", Label)
					InLabelBlock = False
					break
				else:
					Label += commentline
		...
		# Create State Marker:
		#DOC: TODO: If HeadRow is omitted from the table, include the state name automatically.
		#if Label.find('HeadRow') < 0:
		#	index = Label.find('>')	
		#	Label = Label[:index+1] + '<tr id="HeadRow">' + ' <td id="FirstColumn" bgcolor="black" align="center" colspan="1">' \
		#					+  '<font color="white">' + case['State'] + '</font>' +	"</td></tr>" + Label[index+1:]
		#
		#
		...
		for commentline in case['CommentLines']:
			...
			if m is not None:
				...
				DotGraphLines.append(m.group('statename') + ' [label=' + m.group('label') + r' URL="\ref ' + case['State'] + '"];')
		...
		for commentline in case['CommentLines']:
			...
			if m is not None:
				...
				if DestinationDirection is None:
					if Direction is not None:
						DestinationDirection = ":" + Direction.lower()
					else:
...
# Inserted above the previous line!
#						#DOC: TODO: Not sure what this does....
#						if line.find('->'):
#							sys.stderr.write("My def link: '%s'\n" % Destination)
#							if Destination.find('#')>0: 
#								start_ref = Destination.find('#')
#								end_ref = Destination.find(' ',start_ref)
#								if end_ref == -1:
#									end_ref = Destination.find('"',start_ref)
#								if end_ref == -1:
#									end_ref = len(Destination)
#							
#								my_ref = '" URL ="\\ref ' + Destination[(start_ref+1):end_ref] + '"];'
#								Destination = Destination.replace('#','',1)
#								index = Destination.rfind('"')
#								Destination = Destination[:(index)] + my_ref
	...
	DotGraphLines.append('}')
	DotGraphLines.append('@enddot')
	if options.dot_on_stderr:
		sys.stderr.write("\n".join(DotGraphLines[1:-1]))
		sys.stderr.write("\n")
				
	return " ".join(DotGraphLines)

def ProcessStateMachine(lineArray, StateMachinePositions, CommentBlocks, options):
	...
	StateStartRE = re.compile(r'\s*@statemachine(?:\s+(?P<statemachinename>' + iskeyword + r'+))?')
	...
	for LineNumber in range(StateMachinePositions['Start'], StateMachinePositions['End']+1):
		...
		m = StateStartRE.search(ThisLine)
		if m is not None:
			StateMachineName = m.group('statemachinename')
		...
		if Comment is not None:
			...
			if Comment.find('@endstatemachine') != -1:
				...
				DotGraph = ProcessStateCases(options, StateMachineCases, GlobalStates, InitialLines, StateMachineName)
